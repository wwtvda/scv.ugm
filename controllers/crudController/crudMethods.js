...
exports.countStatus = async (Model, req, res) => {
  try {
    // Find document by id
    const result = await Model.find({ status: req.params.status });
    // If no results found, return document not found
    if (!result) {
      return res.status(404).json({
        success: false,
        result: null,
        message: "No document found by this id: " + req.params.status,
      });
    } else {
      // Return success resposne
      return res.status(200).json({
        success: true,
        result,
        message: "we found this document by this id: " + req.params.status,
      });
    }
  } catch (err) {
    // Server Error
